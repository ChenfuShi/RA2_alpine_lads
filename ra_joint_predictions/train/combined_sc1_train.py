...
def train_SC1_model(config, model, model_name, train_set, validation_set ,epochs_before=10,epochs_after=51):
    ...
    if epochs_before > 0:
        ...
        model.fit(train_set,
            epochs = epochs_before, steps_per_epoch = 100, validation_data = validation_set, validation_steps = 5, verbose = 2, callbacks = [saver, tensorboard_callback])
    ...
    saver = CustomSaver(model_name + "after", n = 20)
    model.fit(train_set,
        epochs = epochs_after, steps_per_epoch = 100, validation_data = validation_set, validation_steps = 5, verbose = 2, callbacks = [saver, tensorboard_callback])
