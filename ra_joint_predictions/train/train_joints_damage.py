...
def _get_dataset(config, joint_type, dmg_type, model_type, do_validation = False):
    ...
    if joint_type == 'F':
        ...
        joint_dataset = hands_wrists_dataset(config, model_type = model_type, pad_resize = True, imagenet = False)
        ...
        if do_validation:
            val_dataset = narrowing_test_dataset(config, config.train_fixed_location, model_type = model_type, joint_extractor = df_joint_extractor)
