    ...
    def _predict(test_dataset, outcome_mapping, narrowing_model, erosion_model, no_narrowing_classes = 5, no_erosion_classes = 6):
        for file_info, img in test_dataset:
            ...
            for idx, narrowing_outcome_key in enumerate(narrowing_outcome_keys):
                y_pred = narrowing_model.predict(img_tf)
                y_pred = np.sum(y_pred[idx] * np.arange(no_narrowing_classes), axis = 1)[0]
            ...
            for idx, erosion_outcome_key in enumerate(erosion_outcome_keys):
                y_pred = erosion_model.predict(img_tf)
                y_pred = np.sum(y_pred[idx] * np.arange(no_erosion_classes), axis = 1)[0]
...
def _get_models(predict_params):
    hands_narrowing_model = load_joint_damage_model(predict_params['narrowing_model'], 5)
    wrists_narrowing_model = load_joint_damage_model(predict_params['wrists_narrowing_model'], 5)
    feet_narrowing_model = load_joint_damage_model(predict_params['narrowing_model'], 5)

    hands_erosion_model = load_joint_damage_model(predict_params['hand_erosion_model'], 6)
