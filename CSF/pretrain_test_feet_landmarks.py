...
if __name__ == '__main__':
    ...
    # define model
    model = landmarks_model.densenet_landmarks_model(configuration)
    model.summary()

    logging.info("model prepared")
    # train
    logging.info("starting training")
    pretrain_faces(model,faces,faces_val,configuration,"FACE_pretrain_densenet")
    ...
    logging.info("starting step 2 of feet pre-training")
    joints_model = landmarks_model.densenet_landmarks_model(configuration,12,"weights/FACE_pretrain_densenet_model_250",10)
    joints_model.summary()

    landmark_dataset, landmark_dataset_val = ld_dataset.feet_landmarks_dataset(configuration).create_landmarks_dataset(create_val=True)

    pretrain_landmarks(joints_model,landmark_dataset,landmark_dataset_val,configuration,"FEET_pretrain_densenet")

    logging.info("starting step 2 of hands pre-training")
    joints_model = landmarks_model.densenet_landmarks_model(configuration,26,"weights/FACE_pretrain_densenet_model_250",10)
    joints_model.summary()

    landmark_dataset, landmark_dataset_val = ld_dataset.hands_landmarks_dataset(configuration).create_landmarks_dataset(create_val=True)

    pretrain_landmarks(joints_model,landmark_dataset,landmark_dataset_val,configuration,"HANDS_pretrain_densenet")

    logging.info("training on double pretrained model feet")

    joints_model = landmarks_model.densenet_landmarks_model(configuration,26,"weights/FEET_pretrain_densenet_model_2500",12)
    landmark_dataset, landmark_dataset_val = ld_dataset.hands_landmarks_dataset(configuration).create_landmarks_dataset(create_val=True)
    pretrain_landmarks(joints_model,landmark_dataset,landmark_dataset_val,configuration,"HANDS_train_densenet")

    logging.info("training on double pretrained model hands")

    joints_model = landmarks_model.densenet_landmarks_model(configuration,12,"weights/HANDS_pretrain_densenet_model_2500",26)
    landmark_dataset, landmark_dataset_val = ld_dataset.feet_landmarks_dataset(configuration).create_landmarks_dataset(create_val=True)
    pretrain_landmarks(joints_model,landmark_dataset,landmark_dataset_val,configuration,"FEET_train_densenet")